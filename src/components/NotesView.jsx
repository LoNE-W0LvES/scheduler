...
export default function NotesView({ notes, onAdd, onDelete, onUpdate, exportSettings, importSettings, toast }) {
  const { t } = useLanguage();
  ...
  function saveEdit(id) {
    ...
    setEditId(null); toast('✓ ' + t('noteUpdated'));
  }

  return (
    <div className="px-5 pb-20 fade-up">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <div className="font-kanit text-xl font-bold">{t('notesTitle')}</div>
        {exportSettings && (
          <button
            type="button"
            onClick={() => { exportSettings(); toast('📥 Notes & Settings exported to JSON!'); }}
            className="px-3.5 py-1.5 rounded-xl bg-[#F0EFEB] dark:bg-[#262626] text-[#444] dark:text-[#DDD] text-xs font-semibold hover:bg-[#E2E0D8] transition-all cursor-pointer flex items-center gap-1.5"
          >
        ...
        {notes.map(n => (
          <div key={n.id}
            ...
            <button
              ...
              onClick={() => { onDelete(n.id); toast('🗑 ' + t('deleted')); }}>✕</button>
          </div>
        ))}
      </div>
